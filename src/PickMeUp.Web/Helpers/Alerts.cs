...
public class AlertsAttribute : ActionFilterAttribute
{
    ...
    void OnActionExecutingAlert(ActionExecutingContext context)
    {
        ...
        var tempDataAlerts = controller.TempData[AlertHelper.ALERTS_KEY] as string;

        if (tempDataAlerts != null)
            controller.ViewData[AlertHelper.ALERTS_KEY] = JsonConvert.DeserializeObject<AlertsDto>(tempDataAlerts);
    }
    ...
    void OnActionExecutedAlert(ActionExecutedContext context)
    {
        ...
        {
            var controller = (Controller)context.Controller;
            var viewDataAlerts = controller.ViewData[AlertHelper.ALERTS_KEY];

            if (viewDataAlerts != null)
                controller.TempData[AlertHelper.ALERTS_KEY] = JsonConvert.SerializeObject(viewDataAlerts);
        }
    }
...
public static class AlertsRazorPageExtensions
{
    ...
    {
        var alerts = viewData[AlertHelper.ALERTS_KEY] as AlertsDto;
        if (alerts == null)
        {
...
public static class AlertsControllerExtensions
{
    /// <summary>
    /// Gets the current alerts from the controller's ViewData.
    /// </summary>
    public static AlertsDto? GetCurrentAlerts(this Controller controller)
    {
        return controller.ViewData[AlertHelper.ALERTS_KEY] as AlertsDto;
    }
}